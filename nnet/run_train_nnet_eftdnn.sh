...
env=tf_2_gpu
num_gpus=8
#os.environ["CUDA_VISIBLE_DEVICES"] = "0,3,4,12,14,15"
...
ls $TF_ENV 
echo $env
source activate $TF_ENV/$env
#/root/anaconda3/
#source /root/anaconda3/bin/activate $TF_ENV
$cmd $nnetdir/log/train_nnet.log utils/parallel/limit_num_gpus.sh --num-gpus $num_gpus \
    python nnet/lib/train_eftdnn.py $cmdopts --num-gpus $num_gpus --ps gpu --config $config $train $train_spklist $valid $valid_spklist $nnetdir
#source deactivate
conda deactivate
exit 0
